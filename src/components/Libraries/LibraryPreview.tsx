...
function Container({ children }: { children: ReactNode }) {
    return (
        <div
        className="rounded-md p-3 border-[1px] border-accent flex
...
const LibraryPreview: FunctionComponent<LibraryPreviewProps> = ({ npmName }) => {
    ...
    useEffect(() => {
        ...
        .then(data => {
            ...
            fetch(`https://api.npmjs.org/downloads/point/${createdDate}:${currentDate}/${npmName}`)
            .then(res => res.json())
            .then(downloadsData => {
                ...
                setPackageData({
                    ...
                    readMe: data.readme,
                    latestVersion,
                    fileCount: data.versions[latestVersion].dist.fileCount,
                    time: {
    ...
    return (
        ...
        >
            ...
            >
                <p
                className="font-bold text-center text-sm md:text-base lg:text-lg"
                >
                    {npmName}
                </p>
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            />
                            {packageData!.totalDownloads}
                            <TechTooltip
                            id={npmName + "-downloads"}
                            />
                        </div>
                        ...
                        >
                            <BsFileEarmark
                            aria-label="Blank file icon"
                            />
                            {packageData!.fileCount}
                            <TechTooltip
                            id={npmName + "-file-count"}
                            />
                        </div>
                        ...
                        >
                            {packageData?.latestVersion && `v${packageData!.latestVersion}`}
                        </p>
                        <TechTooltip
                        id={npmName + "-current-version"}
                        />
                    </div>
            ...
            <Container>
                ...
                components={{
                    pre: ({ node, children, ...props }) => (
                        <pre
                        className="w-36 sm:w-44 md:w-56 overflow-x-auto border-[1px] border-secondary rounded-md
